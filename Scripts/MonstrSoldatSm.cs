...
public class MonstrSoldatSm : MonoBehaviour
{
  ...
  private void Update()
  {
    ...
    if (currWeight < 1)
    {
      currWeight += Time.deltaTime * 2.0f;
      if (currWeight > 1)
      {
        currWeight = 1;
  ...
  {
    if (anim.clip != cl /*|| Mathf.Abs(anim[anim.clip.name].speed - sp) > 0.2f*/)
    {
      Debug.LogWarning("ChangeAnim " + anim.clip.name + " to " + cl.name);
      if (currWeight < 1)
      {
        anim[oldClip.name].enabled = false;
      //  Debug.LogWarning("Enabled = false " + anim.clip.name);
      }
      //else
      //{
      oldClip = anim.clip;
      //Debug.LogWarning("old.clip " + anim.clip.name);
      //}
      currWeight = 0;
      anim.clip = cl;
